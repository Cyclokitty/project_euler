function largestProductinaSeries(n) {
    ...
    let arrayCutOff = thousandDigits.length -  n + 1;
    let limiter = 0;
    let start = 0;
    let productArr = [];
    
    while (limiter < arrayCutOff) {
      let multiplier =  n - 1;              
        while (multiplier >= 0) {
          productArr.push(thousandDigits[start + multiplier]);
          multiplier--;
        } 
        let product = productArr.reduce((accum, curr) => {
          return accum * curr;
        }, 1);
